# -*- coding: utf-8 -*-
from treebot import LineClient
from treebot.LineApi import LineTracer
from treebot.LineThrift.ttypes import Message
from treebot.LineThrift.TalkService import Client
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                if msg.text == "ginfo":
                    ...
                    if group.preventJoinByTicket is False: md += "\n\nคำเชิญ URL: ได้รับอนุญาต\n"
                    else: md += "\n\nคำเชิญ URL: ปฏิเสธ\n"
                    if group.invitee is None: md += "\nสมาชิก: " + str(len(group.members)) + "คน\n\nเชิญ: 0คน"
                    else: md += "\nสมาชิก: " + str(len(group.members)) + "คน\nได้รับเชิญ: " + str(len(group.invitee)) + "คน"
                ...
                if msg.text == "เวลา":
                    sendMessage(msg.to, "วันเวลา " + datetime.datetime.today().strftime('%d-%m-%Y %H:%M:%S') + "th")
                if msg.text == "gift":
                    sendMessage(msg.to, text="gift sent", contentMetadata=None, contentType=9)
                if msg.text == "นับ":
                ...
                if msg.text == "อ่าน":
                    ...
                    sendMessage(msg.to,"[กลุ่มรายชื่อ]\n"+ h +"กลุ่มรวม =" +"["+str(len(gid))+"]")
                if msg.text in["แท็ก"]:
                     group = client.getGroup(msg.to)
